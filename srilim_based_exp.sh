...
"$MOSSES_TRN" -root-dir "$WORKING/train/model" -corpus "$CORPUS/train.clean" -f $LANG2 -e $LANG1 -alignment grow-diag-final-and -reordering msd-bidirectional-fe -lm 0:3:"$LM/lm.$LANG2-$LANG1.arpa."$LANG1:0 -external-bin-dir "$HOME/mosesdecoder/tools"



####Tune------------------------------------------------------------------------------------------------------------

"$MOSSES_MERT" "$CORPUS/tune.true.$LANG2" "$CORPUS/tune.true.$LANG1" "$HOME/mosesdecoder/bin/moses" "$WORKING/train/model/model/moses.ini" --mertdir "$HOME/mosesdecoder/bin/" --decoder-flags "-threads 4"
...
"$HOME/mosesdecoder/bin/processLexicalTableMin" -in "$WORKING/train/model/model/reordering-table.wbe-msd-bidirectional-fe.gz" -out "$WORKING/binarised-model/reordering-table"
