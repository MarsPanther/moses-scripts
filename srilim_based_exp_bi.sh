...
"$MOSSES_TRN" -root-dir "$WORKING/train/model" -corpus "$CORPUS/train.clean" -f $LANG1 -e $LANG2 -alignment grow-diag-final-and -reordering msd-bidirectional-fe -lm 0:3:"$LM/lm.$LANG1-$LANG2.arpa."$LANG2:0 -external-bin-dir "$HOME/mosesdecoder/tools"



####Tune------------------------------------------------------------------------------------------------------------

"$MOSSES_MERT" "$CORPUS/tune.true.$LANG1" "$CORPUS/tune.true.$LANG2" "$HOME/mosesdecoder/bin/moses" "$WORKING/train/model/model/moses.ini" --mertdir "$HOME/mosesdecoder/bin/" --decoder-flags "-threads 4"
